...
tf.app.flags.DEFINE_integer('hintLayerIndex', 11, 'The hint layer index')
tf.app.flags.DEFINE_integer('guidedLayerIndex', 5, 'The guided layer index')
tf.app.flags.DEFINE_integer('BottelneckChanelNBR', 8, 'The number of channel on the bottelneck conv')
tf.app.flags.DEFINE_integer('max_pool_btlnk_size', 4, 'The pooling size bottelneck conv')
...
def main(_):
    ...
    val_batch_size = 200
    train_epoch = 10
    init_epoch = 4 if FLAGS.Distillation in {'FitNet', 'FSP', 'FT', 'AB', 'MHGD'} else 0
    ...
    if FLAGS.Distillation == 'FitNet':
        is_teacher = False
    else:
        is_teacher = True
    hintLayerIndex = FLAGS.hintLayerIndex
    guidedLayerIndex = FLAGS.guidedLayerIndex
    max_pool_bottleneck_size = FLAGS.max_pool_btlnk_size
    max_pool_h_w = [max_pool_bottleneck_size, max_pool_bottleneck_size]

    print('guidedLayerIndex is %d' % guidedLayerIndex)
    print('hintLayerIndex is %d' % hintLayerIndex)
    print('BottelneckChanelNBR is %d' % FLAGS.BottelneckChanelNBR)
    ...
    with tf.Graph().as_default() as graph:
        ...
        class_loss, accuracy = MODEL(model_name, FLAGS.main_scope, weight_decay, image, label,
                                     is_training_ph, reuse=False, Distillation=FLAGS.Distillation,
                                     hintLayerIndex=hintLayerIndex,
                                     guidedLayerIndex=guidedLayerIndex,
                                     BottelneckChanelNBR=FLAGS.BottelneckChanelNBR,
        ...
        with tf.Session(config=config) as sess:
            ...
            for step in range(max_number_of_steps):
                ...
                if (step * batch_size) // dataset_len >= init_epoch + epoch_:
                    ...
                    for i in range(val_itr):
                        ...
                        acc = sess.run(accuracy, feed_dict={image_ph: val_batch,
                                                            label_ph: np.squeeze(val_labels[i * val_batch_size:(
                                                                                                                           i + 1) * val_batch_size]),
                ...
                if step % should_log == 0:
                    tf.logging.info('global step %s: loss = %.4f (%.3f sec/step)', str(step).rjust(6, '0'),
...
def MODEL(model_name, scope, weight_decay, image, label, is_training, reuse,
          Distillation, hintLayerIndex, guidedLayerIndex, BottelneckChanelNBR, max_pool_h_w):
    network_fn = nets_factory.get_network_fn(model_name, weight_decay=weight_decay)
    end_points = network_fn(image, label, scope, is_training=is_training, reuse=reuse,
                            Distill=Distillation,
                            hintLayerIndex=hintLayerIndex,
                            guidedLayerIndex=guidedLayerIndex,
                            BottelneckChanelNBR=BottelneckChanelNBR, max_pool_h_w=max_pool_h_w)
